...
//Lo que hacemos es añadir los scripts necesarios para que el cargador de medios de wordpress se muestre
    ...
    function opg_concejal_save($name, $email, $description, $biography, $image, $orden)
    {
        global $wpdb;
        ...
        $save_or_no = $wpdb->insert($wpdb->prefix . 'opg_plugin_concejal', 
            array( 'idConcejal' => NULL, 'name' => esc_js(trim ($name)), 'email' => trim ($email), 'description' => trim ($description), 'biography' => trim ($biography), 'image' => trim ($image), 'orden' => $orden ),
            array( '%d', '%s', '%s', '%s', '%s', '%s', '%d' )
        );
        if (!$save_or_no) {
    ...
    function opg_concejal_update($id, $name, $email, $description, $biography, $image, $orden)
    {
        global $wpdb;
        if (!( isset($name) && isset($description) )) {
            _e('cannot get \$_POST[]');
        ...
        if ( isset($image) && (strlen($image)>0) ){
            $update_or_no = $wpdb->update($wpdb->prefix . 'opg_plugin_concejal', 
                array( 'name' => esc_js(trim ($name)), 'email' => trim ($email), 'description' => trim ($description), 'biography' => trim ($biography), 'image' => trim ($image), 'orden' => $orden ),
                array( 'idConcejal' => $id ),
                array( '%s', '%s', '%s', '%s', '%s', '%d' )
            );
        }
            ...
            $update_or_no = $wpdb->update($wpdb->prefix . 'opg_plugin_concejal', 
                array( 'name' => esc_js(trim ($name)), 'email' => trim ($email), 'description' => trim ($description), 'biography' => trim ($biography), 'orden' => $orden ),
                array( 'idConcejal' => $id ),
                array( '%s', '%s', '%s', '%s', '%d' )
            );
        }        
    ...
    {
        global $wpdb;
        $row1 = $wpdb->get_row("SELECT name, email, description, biography, image, orden  FROM " . $wpdb->prefix . "opg_plugin_concejal  WHERE idConcejal=".$id);
        return $row1;
    }
    ...
    function opg_concejal_getData()
    {
        ...
        $records = $wpdb->get_results( 'SELECT idConcejal, name, email, description, biography, orden, image FROM ' . $wpdb->prefix . 'opg_plugin_concejal ORDER BY orden' );
        if (count($records)>0){
?>
	...
	function opg_plugin_concejal_show_form_in_wpadmin(){
	    ...
	    if(isset($_POST['action']) && $_POST['action'] == 'salvaropciones'){
            ...
            if( strlen($_POST['idConcejal']) == 0 ){
                ...
                opg_concejal_save($_POST['name'], $_POST['email'], $_POST['description'], $_POST['biography'], $_POST['upload_image'], $_POST['orden']);
            }
            else{
                opg_concejal_update($_POST['idConcejal'], $_POST['name'], $_POST['email'], $_POST['description'], $_POST['biography'], $_POST['upload_image'], $_POST['orden']);
            }   
	    }
        else{
        ...
        <form method='post' action='admin.php?page=opg_concejales' name='opgPluginAdminForm' id='opgPluginAdminForm'>
            ...
            <table class='form-table'>
                <tbody>
                    ...
                    <tr>
                        <th><label for='orden'>Orden en el listado</label></th>
                        <td>
                            <input type='text' name='orden' id='orden' value="<?php echo $valueInputOrden ?>" style='width: 30px'>
                        </td>
                    </tr>
